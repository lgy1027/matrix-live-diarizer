...
> **实施状态**: ✅ 全部 30 个 Task 已完成（2026-06）
> 落地 commit: `ecd4249` (项目优化重构)
> 后续 P0/P1 修复:见 `git log --oneline ecd4249..HEAD`
>
> **隐私策略更新**: v0.2+ 起 LLM 支持公网 endpoint（默认仍仅本机,
> 显式 `LLM_ALLOW_PUBLIC=true` 才放行）。详见 [`docs/PRIVACY.md`](../../PRIVACY.md)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 Matrix Live Diarizer 从"转写工具"升级为"产出型生产力工具" — 默认数据不外传，支持 SRT/VTT/Markdown/JSON 导出、说话人统计、可选 LLM 高级能力。

**Architecture:** 保留现有实时流路径几乎不动；新增"产出路径"（exporter + statistics + 持久化 + 前端多页）；LLM 通过可选插件接入，遵守严格隐私护栏（默认仅内网 endpoint，可显式开公网）。
...
| Phase | Tasks | 状态 | 实施 commit |
|---|---|---|---|
| Phase 0: 基础设施 | 0.1-0.3 | ✅ | `ecd4249` |
| Phase 1: 存储层 | 1.1-1.4 | ✅ | `ecd4249` |
| Phase 2: Exporter | 2.1-2.5 | ✅ | `ecd4249` |
| Phase 3: Statistics | 3.1 | ✅ | `ecd4249` |
| Phase 4: API 端点 | 4.1-4.3 | ✅ | `ecd4249` |
| Phase 5: LLM 网关 | 5.1-5.2 | ✅ + 公网扩展 | `ecd4249`, `839ea5b` |
| Phase 6: 实时流改造 | 6.1-6.3 | ✅ | `ecd4249` |
| Phase 7: 前端多页 | 7.x | ⚠️ 部分合并到 SPA | `86f88a6` 等 |

**未勾选 checkbox 的原因**: 实施时通过子代理直接落地,未逐项勾选。
任何需求变更请直接看 `git log` 而非修改此 plan。
