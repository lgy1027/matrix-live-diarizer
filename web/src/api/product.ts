...
export async function getVoiceSampleAudioUrl(personId:string,sampleId:string) { const r=await apiClient.get(`/v1/people/${personId}/samples/${sampleId}/audio`,{responseType:'blob'});return URL.createObjectURL(r.data) }
...
export async function getMeetingAudioUrl(id:string) { const r=await apiClient.get(`/v1/meetings/${id}/audio`,{responseType:'blob'});return URL.createObjectURL(r.data) }
...
export async function uploadVoiceSample(personId:string,file:File) { const data=new FormData();data.append('file',file);return (await apiClient.post(`/v1/people/${personId}/samples`,data,{timeout:0})).data }
export async function downloadMeeting(id:string, format:'markdown'|'srt'|'vtt'|'json') { const r=await apiClient.get(`/v1/meetings/${id}/export`,{params:{format},responseType:'blob'});const url=URL.createObjectURL(r.data);const a=document.createElement('a');a.href=url;a.download=`meeting.${format==='markdown'?'md':format}`;document.body.appendChild(a);a.click();a.remove();window.setTimeout(()=>URL.revokeObjectURL(url),1000) }
