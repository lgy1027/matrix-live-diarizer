...
# ---------------------------------------------------------------------------
# 5. 已存在 speaker 的重复写入保持幂等
# ---------------------------------------------------------------------------
def test_ensure_speaker_existing_label_remains_idempotent(repos):
    """重复确保同一标签时始终返回同一 speaker，且不会产生重复记录。"""
    meetings = repos
    meeting_id = meetings.create(source="live", title="吞吐")
    speaker_id = meetings.ensure_speaker(meeting_id, "SPEAKER_00")

    for _ in range(500):
        assert meetings.ensure_speaker(meeting_id, "SPEAKER_00") == speaker_id
